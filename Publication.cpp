...
namespace lib 
{
    ...
    std::ostream& Publication::write(std::ostream& os) const {
        char title[lib_TITLE_WIDTH + 1] = { 0 };
        std::strncpy(title, m_title, lib_TITLE_WIDTH);
    ...
    std::istream& Publication::read(std::istream& is) {
        // Temporary variables
        char t_title[256]{}, t_shelfId[lib_SHELF_ID_LEN + 1]{};
        ...
        if (conIO(is)) {
            std::cout << "Shelf No: ";
            is.getline(t_shelfId, lib_SHELF_ID_LEN + 1);

            if (strlen(t_shelfId) != lib_SHELF_ID_LEN) {
