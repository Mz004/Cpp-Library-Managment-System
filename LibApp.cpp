...
namespace lib 
{
    ...
    void LibApp::load() {
        ...
        for (i = 0; infile && i < lib_LIBRARY_CAPACITY; i++) {
    ...
    void LibApp::returnPub() {
        ...
        if (!aborted) {
            ...
            if (confirmed) {
                ...
                if (daysLoaned > lib_MAX_LOAN_DAYS) {
                    double penalty = (daysLoaned - lib_MAX_LOAN_DAYS) * 0.5;
                    std::cout << std::fixed << std::setprecision(2);
                    std::cout << "Please pay $" << penalty << " penalty for being " << (daysLoaned - lib_MAX_LOAN_DAYS) << " days late!" << std::endl;
                }
    ...
    void LibApp::newPublication() {
        ...
        if (m_NOLP == lib_LIBRARY_CAPACITY)
        {
